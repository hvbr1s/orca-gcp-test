...
async function main(): Promise<{ duration: number; success: boolean }> {
  ...
  try {
    ...
    // END TIMING HERE - we got the signature back
    ...
    console.log(`🕐 Transaction creation to signature: ${txDuration}ms`);
    ...
    if(swapConfig.useJito){
      try {
        ...
        const completionEntry = `${completionTime}, Completed orca swap via Jito, tx-id: ${transaction_id}, x-request-id: ${xRequestId}, tx-duration: ${txDuration}ms\n`;
        try {
        ...
        const errorEntry = `${errorTime}, Error pushing to Jito: ${error.message}, x-request-id: ${xRequestId}, tx-duration: ${txDuration}ms\n`;
        try {
          fs.appendFileSync('./swap_execution.log', errorEntry);
        } catch (logError) {
          console.warn('Failed to write to execution log:', logError);
        }
        return { duration: txDuration, success: false };
      }
    } else {
      ...
      const completionTime = new Date().toISOString();
      const completionEntry = `${completionTime}, Completed orca swap via Fordefi, tx-id: ${data.id}, x-request-id: ${xRequestId}, tx-duration: ${txDuration}ms\n`;
      try {
        fs.appendFileSync('./swap_execution.log', completionEntry);
      } catch (logError) {
        console.warn('Failed to write to execution log:', logError);
    ...
    return { duration: txDuration, success: true };
    ...
    const errorTime = new Date().toISOString();
    const errorEntry = `${errorTime}, Failed to sign transaction: ${error.message}, tx-duration: ${txDuration}ms\n`;
    try {
      fs.appendFileSync('./swap_execution.log', errorEntry);
    } catch (logError) {
      console.warn('Failed to write to execution log:', logError);
